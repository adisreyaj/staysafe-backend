 ...
 * -----
 * Last Modified: Saturday, 11th April 2020 1:01:35 am
...
(async () => {
  try {
    const ciphertext = (
      await fs.readFile('./firebase_config.staysafe')
    ).toString();
    ...
    await fs.writeFile('./firebase_config.json', JSON.stringify(config));
    process.exit(0);
  } catch (error) {
    console.error(error);
    process.exit(1);
